...
class Main:
    def __init__(self, root):
        ...
        self.wind_data = None
        self.meter_wind = ttk.Meter(self.meter, amountused=self.wind_data, textleft="Wind" ,subtext="Richtung", textright="km/h", subtextstyle="warning",subtextfont="bolt")
        self.meter_wind.grid(column=2, row=1)

        self.view_data = None
        self.view = ttk.Meter(self.meter, amounttotal=100, amountused=self.view_data, textleft="Sicht", subtext="Himmel", textright="km", subtextstyle="warning",subtextfont="bolt")
    ...
    def process_data(self, data):
        if "main" in data:
            ...
            self.meter_temp.configure(amountused=round(self.temp_data))
            self.meter_pressure.configure(amountused=self.datapressure)
            self.meter_wind.configure(amountused=round(self.wind_data), subtext=f"Richtung - {self.output}")
            self.view.configure(amountused=self.view_data, subtext=f"{self.sky_data}")
    ...
    def openweb(self):
        return webbrowser.open_new("http://www.freelance-archery.de")
    

    def create_dropdown(self, button):
        dropdown = ttk.Menu(button, tearoff=False)
        dropdown.add_command(label="FreelanceArchery", command=self.openweb)
